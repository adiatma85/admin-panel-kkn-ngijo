...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.roles.update", [$role->id]) }}" enctype="multipart/form-data">
            ...
            {{-- Scope --}}
            @if (Auth::user()->scope_id != null || $role->scope_id != null)
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('scope_id') ? 'is-invalid' : '' }}" name="scope_id" id="scope_id" required>
                        <option value disabled {{ old('scope_id', $role->scope->id) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                        @foreach($scopes as $scope)
                            <option value="{{ $scope->id }}" {{ old('scope_id', $role->scope->id) === $scope->id ? 'selected' : '' }}>{{ $scope->name }}</option>
                        @endforeach
                    </select>
